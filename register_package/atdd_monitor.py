...
def monitor_str_2_class(monitor_str):
    if monitor_str == "once":
        return MonitorCategory.ONCE
    elif monitor_str == "period":
        return MonitorCategory.PERIOD
    elif monitor_str == "rule":
        return MonitorCategory.RULE
    else:
        raise ValueError("monitor_str should be in ['once', 'period', 'rule']")
...
class MonitorRuleBase:
    ...
    def monitor_trial(self, rule_name, *args, **kwargs):
        raise NotImplementedError


class BttMonitor:
    def __init__(self, shared, report, rule):
        ...
        ##########################
        self.rule_dict = self.rule["rule_dict"]
        self.aggregate_rule = self.rule["aggregate_rule"]
        ##########################
        self.rule_result_dict = None
    ...
    def collect_custom_rule(self, rule_name, *rule_input_args):
        ...
        if rule_name in self.rule_dict:
            rule_config = self.rule_dict[rule_name]
            sys.path.append(rule_config["file_path"])
            class_name = rule_config["class_name"]
            class_module = importlib.import_module(class_name)
            class_constructor = getattr(class_module, class_name)
            class_init_args = rule_config["init_args"]
            if class_init_args is None:
                class_init_args = {}
            rule_instance = class_constructor(**class_init_args)
            rule_result = rule_instance.monitor_rule(*rule_input_args)
