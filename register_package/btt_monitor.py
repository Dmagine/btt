...
from btt_monitor_rule import OnceMonitorRule, PeriodMonitorRule
...
class BttMonitor:
    ...
    def __init__(self, rule_config):
        ...
        self.init_rule_object()
        self.init_intermediate_report()
        self.init_final_report()

    def init_rule_object(self):
        for rule_name, rule_info in self.rule_config.items():
            mode = rule_info["mode"]  #######
            if mode == "once":
                instance = OnceMonitorRule({"rule_name": rule_name})
            elif mode == "period":
                instance = PeriodMonitorRule({"rule_name": rule_name})
            elif mode == "rule":
                ...
                raise ValueError("mode should be in ['once', 'period', 'rule']")
    ...
    def record_metric(self, metric_name, d_args):
        logger.debug(" ".join(["record_metric:", metric_name]))
        if metric_name not in self.rule_instance_dict:
            raise ValueError("metric_name {} should exist in rule_instance_dict".format(metric_name))
        # self.rule_instance_dict[metric_name].task_queue.put(d_args)
        self.rule_instance_dict[metric_name].record_metric(d_args)

    def obtain_metric(self, rule_name, result_idx, mode="wait_all"):
        # mode: immediate_idx, wait_idx, wait_all
        d_args = {"result_idx": result_idx, "mode": mode}
            ...
            with self.intermediate_lock:
                if self.intermediate_expect_idx > self.intermediate_actual_idx:
                    ...
                    for rule_name in self.intermediate_rule_name_list:
                        tmp_d = self.obtain_metric(rule_name, self.intermediate_actual_idx, mode="idx_wait")
                    ...
                    if list(d.keys()) == self.intermediate_rule_name_list:  # ok
                        ...
                        logger.info(" ".join(["_report_intermediate_result report:",
                                              str(self.intermediate_actual_idx), str(d)]))
    ...
    def report_final_result(self):
        ...
        for rule_name in self.rule_name_list:
            ...
            metric_value = self.obtain_metric(rule_name, -1, mode="all_wait")
            if rule_name in self.final_report_rule_name_list:
                d.update({rule_name: metric_value})
        logger.info("report_final_result report: {}".format(d))
